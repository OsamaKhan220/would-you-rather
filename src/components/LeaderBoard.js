...
function LeaderBoard(props) {
  ...
  return Object.values(userRanking).map((user) => {
      ...
      <WhiteCard key={id}>
        <GridList cellHeight={160} className={classes.gridList} cols={16}>
          ...
          <GridListTile cols={3.5}>
            <ScoreBadge
              totalScore={questionAsked + answered}
              width={100}
              height={150}
            />
          </GridListTile>
...
const mapStateToProps = (state) => {
  return {
    userRanking: Object.values(state.users)
      .map((user) => ({
        id: user.id,
        name: user.name,
      ...
      .sort(
        (a, b) => b.answered + b.questionAsked - (a.answered + a.questionAsked)
      ),
  };
};
