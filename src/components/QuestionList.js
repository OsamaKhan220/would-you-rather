      ...
      <NoSsr>
        <div className={classes.root}>
          <AppBar position="static">
            <Tabs
              classes={{ indicator: classes.underline, root: classes.tabColor }}
              variant="fullWidth"
              value={value}
          ...
          {value === 0 && (
            <QuestionStatus questions={unAnsweredQuestion} type="unAnswered" />
          )}
          {value === 1 && (
            <QuestionStatus questions={answeredQuestion} type="answered" />
          )}
        </div>
      </NoSsr>
    );
...
const mapStateToProps = (state) => ({
  ...
  unAnsweredQuestion: Object.values(state.questions)
    .filter((q) => !state.users[state.authedUser].answers.hasOwnProperty(q.id))
    .sort((a, b) => b.timestamp - a.timestamp),

  answeredQuestion: Object.values(state.questions)
    .filter((q) => state.users[state.authedUser].answers.hasOwnProperty(q.id))
    .sort((a, b) => b.timestamp - a.timestamp),
});
